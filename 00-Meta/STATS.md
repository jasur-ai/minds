---
aliases: [Statistika, Statistics, Metrics, Dashboard]
tags: [meta, stats]
sektor: 00-Meta
tur: meta
holat: faol
sarlavha: Statistika
qisqacha: Ecosystem loyihasi umumiy ko'rsatkichlari
---

# 📊 STATS — Umumiy statistika

> *Ecosystem loyihasining umumiy ko'rsatkichlari va statistikasi*

---

## 🎯 Loyiha haqida

| Ko'rsatkich | Qiymat |
|-------------|--------|
| **Loyiha nomi** | Ecosystem — Startup G'oyalar Vaulti |
| **Yaratilgan sana** | 2026-09-02 |
| **Oxirgi yangilanish** | 2026-09-04 (100/100 yakunlandi) |
| **Maqsad** | UZ bozorida REAL daromad beradigan startup g'oyalarni yig'ish va ishlab chiqish |
| **Til** | O'zbek |
| **Format** | Obsidian Markdown |

---

## 📁 Tarkibiy qismlar

### Saqlangan g'oya notalari (5 ta):

| # | Papka | Nota | Tur |
|---|-------|------|-----|
| 1 | 20-Society | ElderlyCare (Parvarish.uz) | 100 qadam (FINAL 60 #1) |
| 2 | 20-Society | ToyByudjet | 100 qadam (FINAL 60 #4) |
| 3 | 04-Education | BogchaTop | 100 qadam (FINAL 60 #5) |
| 4 | 04-Education | RuralEducation (QishloqMaktab.uz) | 100/100 (2026-09-04) |
| 5 | 14-Security | SMB_Cybersecurity (XavfsizBiz.uz) | 100/100 (2026-09-04) |

### Meta fayllar (9 ta):

| Fayl | Vazifa |
|------|--------|
| HOME | Bosh sahifa |
| STATS | Statistika |
| GLOSSARY | Lug'at |
| Chat for AIs and Monitoring | AI 07 ↔ AI 01 aloqa jurnali |
| METHODOLOGY | Yagona metodologiya (7 boqich + UZ ekotizim + checklist) |
| STARTUP-100-STEPS | 100 qadam framework |
| STARTUP-MUAMMOLAR | 5 g'oyaning aniq muammolari + tuzoqlar |
| IDEAS-120-MERGE | 120 → 60 merge + FINAL 60 (manba) |
| REAL-PROBLEMS-UZ | O'zbekistondagi real muammolar (5 g'oya bo'yicha) |

---

## 🗑️ TOZALASH TARIXI (2026-09-04)

| Ko'rsatkich | 1-tozalash | 2-tozalash | Meta tozalash | Jami |
|-------------|-----------|------------|---------------|------|
| **O'chirilgan g'oya notalari** | 59 ta | 11 ta | — | 70 ta |
| **O'chirilgan meta fayllar** | — | 13 ta | 3 ta | 16 ta |
| **Jami o'chirilgan** | 59 ta | 24 ta | 3 ta | **86 ta** |
| **Qolgan g'oya notalari** | 16 ta | 5 ta | 5 ta | — |
| **Qolgan meta fayllar** | 23 ta | 12 ta | **9 ta** | — |

### Meta tozalash (2026-09-04):

- **STARTUP-METHODOLOGY** → METHODOLOGY ga birlashtirildi (dublikat edi)
- **REAL-IDEAS-2026** → barcha 5 g'oyasi rad etilgan (IshTopchi, Payxem bor)
- **UNSOLVED-PROBLEMS** → REAL-PROBLEMS-UZ bilan dublikat

### 2-tozalash sabablari (web-research asosida):

- **MetanNavbat** → Metanchi.uz / Metan Mobile bor
- **FreelancerPayment** → Payxem bor
- **JobPlatform** → IshTopchi (50k+) bor
- **NFCPayment** → Apple Pay / Google Pay chiqyapti, qurilmalarda NFC cheklangan
- **FarmerMarket** → Agro-Market ilovasi bor
- **WaterMonitoring** → UNICEF WQMIS davlat tizimi; sensorli MVP og'ir
- **WaterFilterRental** → Sotuvchilar ko'p
- **EnergySaving** → HET Billing davlat ilovasi bor
- **DigitalInclusion** → NGO/grant — to'lovchi yo'q
- **Wellbeing (Ruhiy.uz)** → 7Cups, Hiwell qisman bor
- **RuralTelemedicine** → HEALZY ishga tushmoqda

---

## 🌳 Soha statistikasi

| Soha | Notalar |
|------|---------|
| 👥 Jamiyat | ElderlyCare, ToyByudjet |
| 📚 Ta'lim | BogchaTop, RuralEducation |
| 🛡️ Xavfsizlik | SMB_Cybersecurity |

### O'chirilgan sohalar (nota qolmagan):

| Soha | Sabab |
|------|-------|
| 🚗 Transport | Metanchi.uz bor |
| 💰 Moliya | Payxem / Apple Pay bor |
| 🌾 Qishloq | Agro-Market bor |
| 🌿 Atrof-muhit | UNICEF / davlat tizimlari bor |
| 🏥 Sog'liq | HEALZY bor |
| 🖥️ Texnologiya | Shablon g'oyalar — haqiqiy emas |
| 💼 Biznes | Shablon g'oyalar — haqiqiy emas |
| 🏠 Ko'chmas mulk | Shablon g'oyalar — haqiqiy emas |
| 🏭 Ishlab chiqarish | Shablon g'oyalar — haqiqiy emas |
| 🛒 Savdo | Uzum/Express24 bor |
| 🎮 Ko'ngilochar | Shablon g'oyalar — haqiqiy emas |
| 📡 Aloqa | Telegram/Slack bor |
| 🏛️ Boshqaruv | Davlat tizimlari bor |
| 🎨 Madaniyat | Shablon g'oyalar — haqiqiy emas |
| ✈️ Sayyohlik | Booking/Airbnb bor |
| ⚽ Sport | Shablon g'oyalar — haqiqiy emas |
| 🚀 Kosmos | Not startup (davlat loyihasi) |

---

## 🔗 Bog'lanishlar statistikasi

| Ko'rsatkich | Qiymat |
|-------------|--------|
| **Ichki linklar** | Yangilangan (faqat saqlangan notalar) |
| **Buzilgan linklar** | 0 (2026-09-04 tekshirilgan) |
| **Eng ko'p bog'langan** | IDEAS-120-MERGE, STARTUP-100-STEPS, Chat for AIs and Monitoring |

---

## ✅ Sifat ko'rsatkichlari

| Ko'rsatkich | Qiymat |
|-------------|--------|
| **Buzilgan linklar** | 0 |
| **Frontmatter** | 100% |
| **100-qadam qilingan** | BARCHA 5 LOYIHA 100/100 (2026-09-04) |
| **GO qarorlari** | 5/5 GO ✅ (ElderlyCare 100 · Bog'chaTop 7.5 · To'yByudjet 7.8 · SMB 8.0 · QishloqMaktab 7.8) |
| **Bosqichlar** | 10/10 × 5 loyiha = 50 bosqich rejasi |
| **Qadamlar** | 500/500 qadam rejasi yozildi |

---

*Oxirgi yangilanish: 2026-09-04 — 5/5 loyiha 100/100 ga yetkazildi (mega-topshiriq 4 yakuni)*