---
aliases: [Dashboard, Bosh sahifa, Start]
tags: [meta, home]
cssclass: dashboard
sektor: 00-Meta
tur: meta
holat: faol
sarlavha: Bosh sahifa
qisqacha: Ecosystem vault navigatsiya markazi
---

# 🌐 ECOSYSTEM — Startup G'oyalar Vaulti

> *2026-09-04 meta tozalashdan so'ng: 5 ta real startup g'oyasi + 9 meta fayl*
> *Faqat UZ bozorida REAL daromad, bo'sh joy va amalga oshirish mumkin bo'lgan g'oyalar saqlandi.*

---

## 🗺️ Navigatsiya

| 🏠 Bosh sahifa | 🗺️ Canvas | 📊 Statistika | 📖 Glossary |
|:---:|:---:|:---:|:---:|
| [[HOME]] | [[Ecosystem.canvas]] | [[STATS]] | [[GLOSSARY]] |

---

## ✅ SAQLANGAN G'OYALAR (5 ta)

| # | Loyiha | Soha | Nega ishlaydi | Fayl |
|---|--------|------|---------------|------|
| 1 | **Parvarish.uz** — keksalar uyda parvarish | Jamiyat | Diaspora to'lovchi, UZ da platforma yo'q | [[20-Society/ElderlyCare]] |
| 2 | **To'yByudjet** — oqilona to'y: byudjet + pudratchilar | Jamiyat | kalki.uz faqat kalkulyator — pudratchilar bozori bo'sh | [[20-Society/ToyByudjet]] |
| 3 | **Bog'chaTop** — bog'cha topish + uy bog'chalari tekshiruvi | Ta'lim | Xususiy/oilaviy bog'cha katalogi bo'sh | [[04-Education/BogchaTop]] |
| 4 | **QishloqMaktab.uz** — qishloq maktab onlayn ta'lim | Ta'lim | Qishloqqa qaratilgan arzon ta'lim bo'sh | [[04-Education/RuralEducation]] |
| 5 | **XavfsizBiz.uz** — KOBM kiberxavfsizlik xizmati | Xavfsizlik | KOBM uchun arzon o'zbekcha xizmat bo'sh | [[14-Security/SMB_Cybersecurity]] |

---

## 📈 LOYIHA PROGRESSI (100 QADAM)

| Loyiha | BOSQICH | Qadam | Holat |
|--------|:------:|:-----:|-------|
| [[20-Society/ElderlyCare]] | 10/10 | 100/100 | 🏆 Etalon — to'liq |
| [[04-Education/BogchaTop]] | 8/10 | 80/100 | ✅ Faol |
| [[20-Society/ToyByudjet]] | 8/10 | 80/100 | ✅ Faol |
| [[14-Security/SMB_Cybersecurity]] | 5/10 | 50/100 | 🔄 Faol |
| [[04-Education/RuralEducation]] | 5/10 | 50/100 | 🔄 Faol |

> *2026-09-04 holatiga ko'ra: ElderlyCare to'liq (etalon), BogchaTop + ToyByudjet QADAM 80 (O'sish rejasi), SMB + RuralEducation QADAM 50 (Sinov rejasi) — barcha bosqich xulosalari yozilgan.*

---

## 🗑️ O'CHIRILGAN G'OYALAR (asosiy sabablar)

| G'oya | Sabab |
|-------|-------|
| MetanNavbat | ⛽ Metanchi.uz / Metan Mobile allaqachon bor |
| Freelancer to'lov | 💸 Payxem aynan shu ishni qilyapti |
| JobPlatform | 📋 IshTopchi (50k+ foydalanuvchi) bor |
| NFCPay | 📱 Apple Pay / Google Pay yaqinda chiqadi + qurilmalarning ko'pida NFC yo'q |
| Fermer.uz | 🌾 Agro-Market ilovasi bor |
| Suv monitoringi | UNICEF WQMIS davlat tizimi bor; sensorli MVP og'ir |
| Suv filtri ijarasi | Sotuvchilar ko'p (purewell, waterfilters.uz) |
| Energiya tejash | HET Billing davlat ilovasi bor |
| Raqamli inkluziya | NGO/grant loyihasi — to'lovchi yo'q |
| Ruhiy.uz | 7Cups, Hiwell qisman bor |
| Qishloq telemeditsina | HEALZY ishga tushmoqda |

---

## 📚 META FAYLLAR (9 ta)

### 🧠 Boshqaruv
| Fayl | Vazifa |
|------|--------|
| [[HOME]] | Bosh sahifa |
| [[STATS]] | Statistika |
| [[GLOSSARY]] | Lug'at |
| [[Chat for AIs and Monitoring]] | AI 07 ↔ AI 01 aloqa jurnali |

### 📐 Metodologiya
| Fayl | Vazifa |
|------|--------|
| [[METHODOLOGY]] | Yagona metodologiya (7 boqich + UZ ekotizim + checklist) |
| [[STARTUP-100-STEPS]] | 100 qadam framework |
| [[STARTUP-MUAMMOLAR]] | 5 g'oyaning aniq muammolari + tuzoqlar |

### 📊 Tadqiqot
| Fayl | Vazifa |
|------|--------|
| [[IDEAS-120-MERGE]] | 120 → 60 merge + FINAL 60 (manba) |
| [[REAL-PROBLEMS-UZ]] | O'zbekistondagi real muammolar (5 g'oya bo'yicha) |

---

## 📊 VAULT HOLATI

| Ko'rsatkich | Qiymat |
|-------------|--------|
| **G'oya notalari** | 5 ta |
| **Meta fayllar** | 9 ta |
| **Jami notalar** | 14 ta |
| **O'chirilgan (1-tozalash)** | 59 ta yaroqsiz shablon nota (2026-09-04) |
| **O'chirilgan (2-tozalash)** | 11 ta g'oya + 13 meta fayl (2026-09-04) |
| **O'chirilgan (meta tozalash)** | 3 ta eskirgan meta fayl (2026-09-04) |

---

## 📋 Foydalanish bo'yicha qo'llanma

### Qanday ishlatish:
1. **Graff ko'rishi** — barcha bog'lanishlarni vizual ko'rish
2. **Canvas** → [[Ecosystem.canvas]] da xaritani ko'rish
3. **AI 01 bilan ishlash** → [[Chat for AIs and Monitoring]] da prompt berish
4. **FINAL 60 ro'yxati** → [[IDEAS-120-MERGE]]

### Asosiy fayllar:
- **100 qadam framework** → [[STARTUP-100-STEPS]]
- **AI 01 bilan aloqa** → [[Chat for AIs and Monitoring]]
- **Muammo tuzoqlari** → [[STARTUP-MUAMMOLAR]]

---

*Oxirgi yangilanish: 2026-09-04 (meta tozalash: 3 meta fayl o'chirildi — jami 86 fayl)*
*MllyCore Ecosystem v3.1*